...
import { formatMoney, formatMoneyDecimal, getBillingAnalytics } from '../../utils/billingApi';
...
const BarChart = ({ data, valueKey = 'value', labelKey = 'label', formatValue = (v) => v, height = 220 }) => {
  const max = Math.max(1, ...data.map((d) => Number(d[valueKey] ?? 0)));
  const barH = 22;
  const gap = 8;
  const chartH = Math.max(height, data.length * (barH + gap) + 8);
  return (
    <svg viewBox={`0 0 600 ${chartH}`} className="w-full" role="img" aria-label="Bar chart">
      {data.map((d, i) => {
        ...
        return (
          <g key={d[labelKey] ?? i}>
            <text x="0" y={y + 15} className="fill-slate-700 text-[11px]" style={{ fontFamily: 'system-ui' }}>
              {String(d[labelKey] ?? '').slice(0, 22)}
            </text>
            <rect x="160" y={y} width={Math.max(2, w)} height={barH} className="fill-emerald-500" rx="3" />
            <text x={170 + Math.max(2, w)} y={y + 15} className="fill-slate-600 text-[11px]" style={{ fontFamily: 'system-ui' }}>
              {formatValue(value)}
            </text>
          </g>
        );
...
const SuperAdminBillingPage = () => {
  ...
  const packageData = useMemo(() => {
    ...
    return (data?.byPackage ?? []).map((row) => ({
      label: recentByPkg.get(row.packageId) ?? row.packageId?.slice(0, 8) ?? 'pkg',
      value: Number(row._sum?.amount ?? 0),
      count: row._count?._all ?? 0,
    }));
  }, [data]);
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600">Revenue, platform fees and active subscriptions across the platform.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {providerData.length === 0 ? (
            ...
            <div className="mt-3">
              <BarChart data={providerData} formatValue={(v) => formatMoneyDecimal(v, currency)} />
            </div>
          )}
        </section>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {packageData.length === 0 ? (
            ...
            <div className="mt-3">
              <BarChart data={packageData} formatValue={(v) => formatMoneyDecimal(v, currency)} />
            </div>
          )}
        </section>
      </div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              ...
              {(data?.recent ?? []).length === 0 ? (
                <tr><td colSpan={8} className="py-6 text-center text-sm text-slate-500">No transactions in this range.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-3xl font-bold text-emerald-600">{(data?.activeSubscriptions ?? 0).toLocaleString()}</p>
      </section>
    </div>
  );
};
